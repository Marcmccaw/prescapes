...
async function createRecurringCalendarEvent(metadata, subscriptionId, token) {
...
// Use the same Google service account token as Calendar, but with Firestore scope added
const GOOGLE_SCOPES = [
...
async function writeToFirestore(collection, docId, data, token) {
...
async function saveSubscriberToFirestore(metadata, subscriptionId, customerEmail, token) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Get a single auth token covering both Calendar and Firestore scopes
    const token = await getGoogleAccessToken();

    await Promise.allSettled([
      createRecurringCalendarEvent(metadata, session.subscription, token),
      saveSubscriberToFirestore(metadata, session.subscription, customerEmail, token)
    ]).then(results => {
